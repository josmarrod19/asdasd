...
def lee_datos_extranjeros(fichero):
    ...
    with open(fichero, encoding="utf-8") as f:
        lector = csv.reader(f)
        next(lector)
        return [DatosExtranjeros(distrito,seccion,barrio,pais,int(hombres),int(mujeres)) 
                for distrito,seccion,barrio,pais,hombres,mujeres
                in lector]
    ...
    with open(fichero, encoding='utf-8') as f:
        ...
        for distrito, seccion, barrio, pais, hombres,mujeres in lector:
            hombres=int(hombres)
            mujeres=int(mujeres)
            tupla=DatosExtranjeros(distrito,seccion,barrio,pais, 
                                   hombres, mujeres)
...
def numero_nacionalidades_distintas(registros):
    ...
    '''    
    conj = set(r.pais for r in registros)
    return len(conj)
...
def secciones_distritos_con_extranjeros_nacionalidades(registros, paises):
    ...
    '''
    conj= set((r.distrito,r.seccion) for r in registros 
               if  r.pais in paises)
    return sorted(conj)
...
def total_extranjeros_por_pais(registros):
    ...
    for r in registros:
        ...
        if clave not in dicc:
            dicc[clave]= r.hombres+r.mujeres
        else:
            dicc[clave]+= r.hombres+r.mujeres
...
def agrupar_por_barrio(registros):
    ...
    for r in registros:
        ...
        clave=r.barrio
        if clave not in dicc:
            dicc[clave]= set(r.pais)
        else:
            dicc[clave].add(r.pais)    
    return dicc
...
def contar_por_barrio(registros,tipo=None):
    ...
    for r in registros:
        ...
        if clave not in dicc:
            dicc[clave]= 0
        if tipo =='Hombres':    
            dicc[clave]+=r.hombres
        elif tipo == 'Mujeres':
            dicc[clave]+=r.mujeres
        else:
            dicc[clave]+=r.hombres+r.mujeres
